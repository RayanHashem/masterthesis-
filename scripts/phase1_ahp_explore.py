...
threshold_min ,weights ,label ='supply',supply_beds =None ):
        ...
        try :
            ...
            if nodes_covered :
                ...
                if points :
                    ...
                    pts_proj =pts_gdf .to_crs ('EPSG:32636')
                    buffered =pts_proj .geometry .buffer (200 )
    ...
    if supply_beds is not None and len (weights )==4 :
        ...
        d_reset =d .reset_index (drop =True ).reset_index ()
...
if len (grid_cells_gdf )>0 and 'min_travel_time_min'in grid_cells_gdf .columns :
    try :
        grid_cells_gdf ['pop_density']=grid_cells_gdf ['population']
        ...
        def _find_candidates_by_rule (gdf ,districts ,qual_mask =None ):
            ...
            for _clbl ,_grp in _qual .groupby ('_cluster'):
                ...
                _radius_m =int (max (1500 ,min (6000 ,float (_spread .max ())*0.6 )))if len (_spread )>0 else 1500 
                ...
                _mean_tt =float (_grp ['min_travel_time_min'].dropna ().mean ())if _grp ['min_travel_time_min'].notna ().any ()else np .nan 
                _mean_score =float (_grp ['ahp_score'].mean ())if 'ahp_score'in _grp .columns else 0.0 
