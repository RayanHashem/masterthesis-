"""
Lebanese EMS placement rules and operational standards.

Authoritative sources (cited at runtime by the Copilot):

[LRC-9] Lebanese Red Cross / IFRC, "International Network Plan 2023, Lebanon":
        target = respond to 80% of emergency calls within 9 minutes.
[LRC-13] Same source, secondary target = respond to 85% of calls within 13 minutes.
[MoPH-473] Lebanese Ministry of Public Health, Karar (Decision) No. 473/2021:
        licenses EMS / ambulance operators. Spatial spacing is not publicly codified.
[EU-8]  Widely adopted European critical-patient response target = 8 minutes.
[WHO]   WHO Clinical Services & Systems framework: ambulance base station = facility
        equipped for personnel + restocking; primary care typically serves ~5,000 pop.

Where Lebanese law is silent on spatial placement, we apply transparent demo
heuristics derived from the 9-min response target. These are clearly labeled
to the user as heuristics, not statutes.
"""
...
def is_urban(governorate: str) -> bool:
    """Whether to apply urban or rural thresholds for a given governorate."""
    return (governorate or "").replace(" ", "") in {g.replace(" ", "") for g in URBAN_GOVERNORATES}


def rules_summary_markdown() -> str:
    """Human-readable summary the LLM can paste back to the user."""
