"""
EMS Copilot backend (FastAPI + optional Ollama LLM) — AHP edition.

Design rules:
- Numbers ALWAYS come from Python computing on the actual GeoDataFrame and
  the AHP scores produced by `phase1_ahp_explore.py`.
- The LLM only explains / phrases results, never invents stats.
- Spatial questions ("where can I open a station in X?") run a real spatial
  tool before reaching the LLM.
- Lebanese regulatory references come from `lebanon_ems_rules.py`.

Run:
    uvicorn copilot_server:app --host 127.0.0.1 --port 8000

If Ollama (qwen2.5:3b) is running locally, answers are phrased in natural
language. Otherwise the Copilot falls back to deterministic templated
answers — still fully functional, just less conversational.
"""
...
# ---------------------------------------------------------------------------
# Load base data once at startup
# ---------------------------------------------------------------------------
...
# Detect lat/lon columns case-insensitively
...
# Population raster sampling (sparse — same idea as the dashboard script)
...
# Merge in AHP scores (sidecar JSON written by phase1_ahp_explore.py).
# This is optional — if the sidecar is missing, the Copilot still works in
# basic mode (no AHP-aware answers).
...
def _alert(row):
    """Quick categorical alert used when AHP priority is unavailable."""
    if row.get("ahp_priority"):
        return row["ahp_priority"]  # High / Medium / Low
...
# ---------------------------------------------------------------------------
# Spatial tool: assess a candidate location for a new EMS center
# ---------------------------------------------------------------------------
def geocode_place(query: str) -> Optional[Dict[str, Any]]:
    """Geocode via OSM Nominatim. Returns lat/lon (+optional bbox)."""
...
def _underserved_alternatives(n: int = 5) -> List[Dict[str, Any]]:
    """Top districts by AHP priority + population — what the dashboard would
    recommend over an arbitrary candidate."""
...
def assess_placement(place_query: str) -> Dict[str, Any]:
    """For 'I want to open an EMS center in X' questions."""
...
def tool_top_priority(n: int = 5) -> Dict[str, Any]:
    """Top-N districts ranked by AHP score (or by population if no AHP data)."""
    df = districts_agg.copy()
    if df["ahp_score"].notna().any():
        ranked = df.sort_values("ahp_score", ascending=False).head(n)
        sort_key = "ahp_score"
...
def route_intent(message: str) -> Optional[Dict[str, Any]]:
    msg = message.strip()
    ...
    if NATIONAL_RE.search(msg) and any(w in msg.lower() for w in ("overview", "summary", "total", "stats", "statistics")):
...
7. Keep answers under 200 words. No emojis. No markdown headers, only bold and lists.
"""


def call_ollama(user_message: str, tool_result: Optional[Dict[str, Any]]) -> str:
...
# ---------------------------------------------------------------------------
# Fallback formatter (used when the LLM is unavailable)
# ---------------------------------------------------------------------------
...
def chat(req: ChatRequest):
    ...
    if req.use_llm:
        answer = call_ollama(req.message, tool_result)
        ...
        if tool_result is not None:
            ...
            answer = ("I can answer questions about district populations, EMS stations, "
